...
import { addStyle, cellValueString } from '.'
import type { Range } from '../table-renderer'
import type { Cells, TableData, IndexDataCell } from '.'
...
export function copy(from: CopyCells | null, to: CopyCells | null, autofill = false) {
    if (!from || !to) return
    ...
    to.range.each((r, c) => {
        ...
        const colOffset = toIndex[1] % (cols + 1)
        const rowOffset = toIndex[0] % (rows + 1)
...
function copyToDataCell(
    ...
    const fromCell = from.cells.get(fr, fc)
    if (fromCell !== null && fromCell[2] !== undefined && fromCell[2] !== null) {
        ...
        if (newCell instanceof Object) {
            ...
            // style
            if (newCell.style !== undefined && from.cells !== to.cells) {
            ...
            if (position !== undefined && n !== undefined) {
                ...
                if (newCell.formula) {
                    ...
                    if (!Renders.use().getRender(cellTypeGetter(newCell)).disableAutoFillAction) {
                        newCell.value = String(getCellValue(newCell.value, n))
                    }
                }
            }
        } else {
...
function subtractValue(a1: IndexDataCell | null, a2: IndexDataCell | null) {
    if (
        ...
        if (v1 !== '' && v2 !== '') {
            return Number.parseInt(v2) - Number.parseInt(v1)
        }
    }
    return 1
}
...
function getCellValue(v: string | number, n: number) {
    if (typeof v === 'string') {
        return v.replace(/(\d+$)|((\d+)\D+$)/g, (word) => {
            return word.replace(/\d+/, (w) => `${Number.parseInt(w) + n}`)
        })
    }
    return v + n
}
