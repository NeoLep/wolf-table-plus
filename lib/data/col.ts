...
import type { DataCol, TableData } from '.'
...
export function colsWidth(data: TableData, min?: number, max?: number) {
    ...
    if (arguments.length === 1) {
        ...
        for (const key in cols) {
            if (key !== 'len') {
                const h = colWidth(data, Number.parseInt(key, 10))
                if (h > 0) {
                    total += h
                    total -= data.colWidth
                }
    ...
    return sum(min !== undefined ? min : 0, max !== undefined ? max : cols.len, (i) =>
        colWidth(data, i),
    )
}
...
export function getMaxColIndexHasValue(data: TableData) {
    let maxCol = 0
