    ...
    constructor(
        ...
        this._ = h('div', `${stylePrefix}-resizer ${type}`).append(
            (this._hover = h('div', 'hover').on('mousedown.stop', (evt) =>
                mousedownHandler(this, evt as MouseEvent),
            )),
...
function mousedownHandler(resizer: Resizer, evt: MouseEvent) {
    let prevEvent: MouseEvent | null = evt
    ...
    const upHandler = () => {
        unbind(document.body, 'mousemove', (e) => moveHandler(e as MouseEvent))
        unbind(document.body, 'mouseup', upHandler)
        prevEvent = null
        _line.hide()
        _.hide()
        if (_cell && distance != 0) {
    ...
    bind(document.body, 'mousemove', (e) => moveHandler(e as MouseEvent))
    bind(document.body, 'mouseup', upHandler)
}
