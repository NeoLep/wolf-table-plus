 ...
 * @returns
 */
function autoMove(t: Table, range: Range | null | undefined, oldRange?: Range | null) {
    if (!range) return
            ...
            if (oldRange) {
                ...
                if (range.endRow === oldRange.endRow) {
                    ...
                    if (range.startRow < oldRange.startRow) {
                        // up+
                        if (range.startRow > range2.endRow && range.startRow < range4.startRow) {
                            _vScrollbar.scrollBy(-t.rowsHeight(range.startRow, range4.startRow))
                        }
                    } else if (
                        range.startRow > oldRange.startRow && // up-
                        range.startRow >= range4.endRow
                    ) {
                    ...
                    if (range.endRow > oldRange.endRow) {
                        ...
                        range.endRow < oldRange.endRow && // down-
                        range.endRow < range4.startRow
                    ) {
                ...
                } else if (range.startRow > range2.endRow && range.startRow < range4.startRow) {
                    _vScrollbar.scrollBy(-t.rowsHeight(range.startRow, range4.startRow))
                }
            }
        }
            ...
            if (oldRange) {
                ...
                if (range.endCol === oldRange.endCol) {
                    ...
                    if (range.startCol < oldRange.startCol) {
                        // left+
                        if (range.startCol > range2.endCol && range.startCol < range4.startCol) {
                            _hScrollbar.scrollBy(-t.colsWidth(range.startCol, range4.startCol))
                        }
                    } else if (
                        range.startCol > oldRange.startCol && // left-
                        range.startCol >= range4.endCol
                    ) {
                    ...
                    if (range.endCol > oldRange.endCol) {
                        ...
                        range.endCol < oldRange.endCol && // right-
                        range.endCol < range4.startCol
                    ) {
                ...
                } else if (range.startCol > range2.endCol && range.startCol < range4.startCol) {
                    _hScrollbar.scrollBy(-t.colsWidth(range.startCol, range4.startCol))
                }
            }
        }
